...
(function (root, factory) {
  ...
  ProtectedCallManager.prototype = {
    ...
    noInsideIteration: function () {
      return (
        this.iterationStatement[this.iterationStatement.length - 1].length === 0
      );
    },
    ...
    breakOutside: function () {
      if (
        this.isInProtectedCallContext() &&
        this.noInsideIteration() &&
        this.noInsideSwitch()
      ) {
  ...
  function compileIterationStatement(statement, compiledLabel) {
    ...
    switch (statement.type) {
      case "ForStatement":
        compiledIterationStatement = compileForStatement(
          statement,
          compiledLabel
        );
        break;
      case "WhileStatement":
        compiledIterationStatement = compileWhileStatement(
          statement,
          compiledLabel
        );
        break;
      case "DoWhileStatement":
        compiledIterationStatement = compileDoWhileStatement(
          statement,
          compiledLabel
        );
        break;
      case "ForInStatement":
        compiledIterationStatement = compileForInStatement(
          statement,
          compiledLabel
        );
        break;
  ...
  function isNumericCompoundAssignmentExpressionWith(expression, variables) {
    if (expression !== null && isCompoundAssignment(expression)) {
      ...
      if (
        ...
        variables.indexOf(expression.left.name) > -1
      ) {
  ...
  function mayBeNumericFor(statement) {
    ...
    if (possibleNumericForVariable.length > 0) {
      ...
      if (
        isComparisonExpressionWith(statement.test, possibleNumericForVariable)
      ) {
        // Update is an UpdateExpression of the numeric for variable
        if (
          isUpdateExpressionWith(statement.update, possibleNumericForVariable)
        ) {
          return true;
        }
        if (
          isNumericCompoundAssignmentExpressionWith(
            statement.update,
            possibleNumericForVariable
          )
        ) {
          return true;
        }
  ...
  function compileLabeledStatement(statement) {
    ...
    if (isIterationStatement(statement.body)) {
      compiledLabeledStatement.push(
        compileIterationStatement(statement.body, compiledLabel)
      );
    } else {
  ...
  function compileSwitchStatement(statement) {
    ...
    if (cases.length > 0) {
      ...
      for (i = 0; i < cases.length; ++i) {
        ...
        compiledSwitchStatement.push(
          compileListOfStatements(cases[i].consequent)
        );
        compiledSwitchStatement.push("\n");
        compiledSwitchStatement.push("_into = true;\n");
        compiledSwitchStatement.push("end\n");
  ...
  function compileTryStatementFlavored(statement, esprima) {
    // @number
    var hasHandler = esprima
      ? statement.handlers.length > 0
    ...
    if (hasFinalizer || may.mayReturn || may.mayBreak || may.mayContinue) {
      ...
      if (may.mayBreak && may.mayContinue) {
        compiledTryStatement.push(
          "if _return == _break then break; elseif _return == _continue then goto _continue end\n"
        );
      } else if (may.mayBreak) {
        compiledTryStatement.push("if _return == _break then break; end\n");
      } else if (may.mayContinue) {
        compiledTryStatement.push(
          "if _return == _continue then goto _continue end\n"
        );
      }
    ...
    if (hasHandler) {
      ...
      if (may.mayBreak && may.mayContinue) {
        compiledTryStatement.push(
          "if _creturn == _break then break; elseif _creturn == _continue then goto _continue end\n"
        );
      } else if (may.mayBreak) {
        compiledTryStatement.push("if _creturn == _break then break; end\n");
      } else if (may.mayContinue) {
        compiledTryStatement.push(
          "if _creturn == _continue then goto _continue end\n"
        );
      }
      ...
      case "UnaryExpression":
        ...
        if (expression.operator === "!") {
          ...
          compiledUnaryExpressionStatement.push(
            compileUnaryExpression(expression, meta)
          );
          compiledUnaryExpressionStatement.push(");");
  ...
  function compileCompoundAssignmentBinaryExpression(
    left,
    right,
    operator,
    metaLeft,
    ...
    switch (operator) {
      ...
      case "+=":
        compiledCompoundAssignmentBinaryExpression.push(
          compileAdditionOperator(left, right, metaLeft, metaRight, meta)
        );
        break;
      case "-=":
        pushSimpleBinaryExpression(
          compiledCompoundAssignmentBinaryExpression,
          " - ",
          left,
          right
        );
  ...
  function compileCompoundAssignmentNoEval(expression) {
    ...
    compiledAssignmentBinaryExpression.push(" = ");
    compiledAssignmentBinaryExpression.push(
      compileCompoundAssignmentBinaryExpression(
        left,
        right,
        expression.operator,
        metaLeft,
  ...
  function compileLogicalExpression(expression, meta) {
    ...
    if (
      ...
      return compileLogicalExpressionLeftIdentifierOrLiteral(
        expression,
        compiledLeft,
        compiledRight
      );
    } else {
      return compileGenericLogicalExpression(
        expression,
        compiledLeft,
        compiledRight
      );
  ...
  function compileLogicalExpressionLeftIdentifierOrLiteral(
    expression,
    compiledLeft,
    compiledRight
  ) {
  ...
  function compileGenericLogicalExpression(
    expression,
    compiledLeft,
    compiledRight
  ) {
  ...
  function compileUnaryExpression(expression, meta) {
    ...
    var metaArgument = {};
    var compiledExpression = compileExpression(
      expression.argument,
      metaArgument
    );
    ...
    if (expression.prefix) {
      ...
      switch (expression.operator) {
        ...
        case "!":
          ...
          compiledUnaryExpression.push(
            compileBooleanExpression(expression.argument)
          );
          if (meta) {
            meta.type = "boolean";
          }
  ...
  function compileComparisonOperator(
    left,
    right,
    operator,
    metaLeft,
    ...
    // Raw comparison
    if (
      (metaLeft.type === "string" && metaRight.type === "string") ||
      (metaLeft.type === "number" && metaRight.type === "number")
    ) {
  ...
  function pushSimpleBinaryExpression(
    compiledBinaryExpression,
    operator,
    left,
    right
  ...
  function compileConditionalExpression(expression, meta) {
    ...
    compiledConditionalExpression.push(
      compileBooleanExpression(expression.test)
    );
    compiledConditionalExpression.push(" then return ");
    compiledConditionalExpression.push(
      compileExpression(expression.consequent, metaConsequent)
    );
    compiledConditionalExpression.push("; else return ");
    compiledConditionalExpression.push(
      compileExpression(expression.alternate, metaAlternate)
    );
    compiledConditionalExpression.push("; end end)()");

    if (meta) {
      if (
        metaConsequent.type === metaAlternate.type &&
        metaConsequent.type !== undefined
      ) {
  ...
  function compileMemberExpression(expression, meta) {
    ...
    if (expression.computed) {
      ...
      if (compiledProperty !== expression.property.name) {
        compiledMemberExpression.push('["');
        compiledMemberExpression.push(
          sanitizeLiteralString(expression.property.name)
        );
        compiledMemberExpression.push('"]');
      } else {
        compiledMemberExpression.push(".");
  ...
  function compileFunction(fun) {
    ...
    if (locals.length > 0) {
      ...
      var compiledLocalsDeclaration = buildLocalsDeclarationString(
        locals,
        compiledParams
      );
  ...
  function sanitizeIdentifier(id) {
    ...
    return id
      ...
      .replace(/[\u0080-\uFFFF]/g, function (c) {
        // Latin-1 Supplement is allowed in JS var names, not yet in Lua
        // @string
  ...
  function sanitizeRegExpSource(str) {
    return str
      ...
      .replace(/"/g, '\\"') // escape double quotes
      .replace(
        /\\\\u([0-9A-Fa-f][0-9A-Fa-f][0-9A-Fa-f][0-9A-Fa-f])/g, // convert "\uXXXX" string to character, then handle it
